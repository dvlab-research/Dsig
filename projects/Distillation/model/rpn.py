...
class DistillRPN(nn.Module):
    ...
    def losses(
            ...
            teacher_objectness_logits=None,
            teacher_anchor_deltas=None
        ...
        if teacher_objectness_logits is not None:
            T = 1
            kd_rpn_cls_loss = F.kl_div(F.log_softmax(cat(pred_objectness_logits, dim=1)[valid_mask].unsqueeze(0) / T, dim=1),
                                       F.softmax(cat(teacher_objectness_logits, dim=1)[valid_mask].unsqueeze(0) / T, dim=1),
                                       reduction="sum") * (T * T)

            kd_rpn_loc_loss = smooth_l1_loss(
                cat(pred_anchor_deltas, dim=1)[pos_mask], cat(teacher_anchor_deltas, dim=1)[pos_mask], self.smooth_l1_beta, reduction="sum"
            )
        ...
        if teacher_objectness_logits is not None:
            kd_losses = {'loss_d_rpn_cls': kd_rpn_cls_loss/normalizer, 'loss_d_rpn_loc': kd_rpn_loc_loss/normalizer}
    ...
    def forward(
        ...
        if teacher_logits is not None:
            teacher_objectness_logits = [
                # Reshape: (N, A, Hi, Wi) -> (N, Hi, Wi, A) -> (N*Hi*Wi*A, )
                x.permute(0, 2, 3, 1).flatten(1)
                for x in teacher_logits['pred_obj_logits']
            ]

            teacher_anchor_deltas = [
                # (N, A*B, Hi, Wi) -> (N, A, B, Hi, Wi) -> (N, Hi, Wi, A, B) -> (N, Hi*Wi*A, B)
                x.view(x.shape[0], -1, self.anchor_generator.box_dim, x.shape[-2], x.shape[-1])
                    .permute(0, 3, 4, 1, 2)
                    .flatten(1, -2)
                for x in teacher_logits['pred_anchor_deltas']
            ]
        else:
            teacher_objectness_logits = None
            teacher_anchor_deltas = None
        ...
        if self.training:
            ...
            losses, distill_losses = self.losses(
        ...
        return proposals, losses, distill_losses
